...
bool DirectXManager::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool isFullscreen, float screenDepth, float screenNear)
{
	...
	// Set the width and hright of the back buffer
	swapChainDesc.BufferDesc.Width = screenWidth;
	swapChainDesc.BufferDesc.Height = screenHeight;

	// Set regular 32-bit serface for the back buffer
	...
	// Initialize the world matrix to the identity matrix  <--- Camera
	...
	*/
	...
	result = m_device->CreateBlendState(&blendStateDesc, &m_alphaEnabledBlendingState);
	if (FAILED(result))
	{
		return false;
	}
...
void DirectXManager::EnableZBuffer(bool enable)
{
	if (enable)
		m_deviceContext->OMSetDepthStencilState(m_depthStencilState, 1);
	else
...
void DirectXManager::EnableBlendState(bool enable)
{
	float blendFactor[4];
	blendFactor[0] = 0.0f;
	blendFactor[1] = 0.0f;
